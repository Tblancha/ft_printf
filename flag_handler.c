...
/*   Updated: 2019/10/12 01:26:22 by tblancha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char
	*correction_precision_padding_flag_zero(char *data_to_write,
...
static char
	*handler_signe_or_flag_plus(t_buff *buffer_s, char *data,
	t_info_data_to_write *opt)
{
	static int	check = 0;

	if ((opt->flag_bin & flag_to_bin(25) && (opt->flag_bin & flag_to_bin(26))
				&& check == 0) || (opt->flag_bin & flag_to_bin(25)
					&& check == 1 && !(opt->flag_bin & flag_to_bin(26))))
	{
		if (data[0] == '-')
			buffer_s->len_total += ft_putchar_buff(*data++, buffer_s->buff);
		else
			buffer_s->len_total += ft_putchar_buff('+', buffer_s->buff);
		check += 9;
	}
	else if ((opt->type == 8 || opt->type == 6) && check == 1)
	{
		if (data[0] == '-')
			buffer_s->len_total += ft_putchar_buff(*data++, buffer_s->buff);
	}
	else if (check == 0 && (opt->flag_bin & flag_to_bin(_TOKEN_FLAG_ZERO)))
		if (data[0] == '-')
		{
			buffer_s->len_total += ft_putchar_buff(*data++, buffer_s->buff);
			++check;
...
void
	apply_flag_router(t_buff *buffer_s, char *data_to_write,
	t_info_data_to_write *option)
{
	...
	if (option->flag_bin & flag_to_bin(_TOKEN_FLAG_ZERO))
		buffer_s->len_total += ft_repeat_char_buff('0',
				option->padding, buffer_s->buff);
	else if (!(option->flag_bin & flag_to_bin(_TOKEN_FLAG_MINUS)))
		buffer_s->len_total += ft_repeat_char_buff(' ', option->padding,
				buffer_s->buff);
	data_to_write = handler_signe_or_flag_plus(buffer_s, data_to_write, option);
	if (option->flag_bin & flag_to_bin(_TOKEN_FLAG_DIESE)
			&& !(option->flag_bin & flag_to_bin(_TOKEN_FLAG_ZERO)))
		buffer_s->len_total += prefix_router(buffer_s->buff,
				option, data_to_write);
...
int
	prefix_router(char *buff, t_info_data_to_write *option,
	char *data_to_write)
{
	if ((data_to_write[0] == 0 || data_to_write[0] == '0')
			&& (option->type != _TOKEN_TYPE_OCTAL
				&& option->type != _TOKEN_TYPE_POINTER))
		return (0);
	...
	else if (option->type == _TOKEN_TYPE_HEX_MIN
			|| option->type == _TOKEN_TYPE_POINTER)
	{
		buff_manager("0x", buff);
		return (2);
